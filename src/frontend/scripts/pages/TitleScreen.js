...
export default class TitleScreen {
  ...
  createElement() {
    ...
    this.ecopyright = qutor(".copyright", this.el)
    this.emenu = qutor(".main-menu", this.el)
    this.ewelcome = qutor(".user", this.el)
    this.euser = qutor("[data-username]", this.el)
    this.euser.innerText = db.me.username
  }
  async clickListener() {
    await waittime(100)
    this.game.pause()
    const btnStart = qutor(".btn-start", this.el)
    const btnSetting = qutor(".btn-setting", this.el)
  ...
  navKeyListener() {
    const buttons = Array.from(this.emenu.querySelectorAll(".btn"))
