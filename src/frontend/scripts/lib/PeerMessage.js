...
class PeerMessage {
  ...
  prologueSkip(data) {
    if (db.pmc?.id === "prologue") {
      db.pmc.updateSkipped(data.from)
      return
    }
  ...
  addStates(data) {
    ...
    data.states.forEach((state) => (SaveList[state] = true))
    if (data.text) {
  ...
  removeStates(data) {
    if (!data.states) return
    if (!Array.isArray(data.states)) return
    data.states.forEach((state) => delete SaveList[state])
  }
  ...
  playerMove(data) {
    const remotePlayer = this.game.map.gameObjects[`crew_${data.from}`]
  ...
  playerMapChange(data) {
    const remotePlayerObject = this.game.map.gameObjects[`crew_${data.from}`]
    const peerData = peers.get(data.from)
