...
export default class FingerprintCloner {
  constructor(config) {
    this.id = "minigame"
  ...
  createElement() {
    ...
    this.target = qutor("[data-target]", this.el)
    this.connections = qutor("[data-connections]", this.el)
  }
  async tutorPopup() {
    this.isLocked = true
...
<li>Cloner memiliki 3 koneksi yang harus diselesaikan</li>
    ...
    await new Promise((resolve) => {
      img.onload = () => {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          const arrow_left = qutor(".aleft.arrow", imgcover)
          arrow_left.onclick = () => {
          ...
          const arrow_right = qutor(".aright.arrow", imgcover)
          arrow_right.onclick = () => {
            audio.emit({ action: "play", type: "ui", src: "hack_selector", options: { id: Date.now().toString() } })
  ...
  async checkConnections() {
    ...
    if (conCorrect >= 8) {
      ...
      qutor(".desc", fingscan).innerHTML = "CONNECTED"
      qutor(".desc", fingscan).classList.add("cyan")
      qutor(".box", fingscan).classList.add("correct")
      this.imgpar++
      this.nextConn()
      await waittime(1500)
  ...
  navKeyListener() {
    this.navKeyHandler = (e) => {
      ...
      const allCovers = Array.from(this.connections.querySelectorAll(".imgcover"))
      ...
      if (key === "ArrowUp" || key === "ArrowDown") {
        ...
        if (activeCover) {
          ...
          activeCover.querySelector(arrow)?.click()
        }
      }
    }
