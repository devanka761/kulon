...
export default class Phone {
  ...
  createElement() {
    ...
    const euname = qutor(".profile .detail .user .username", this.el)
    const euserid = qutor(".profile .detail .user .userid .uid", this.el)
    const eskin = qutor(".profile .detail .avatar .hero", this.el)
  ...
  async btnListener() {
    const btnClose = qutor(".profile .btn-close", this.el)
    btnClose.onclick = () => {
    ...
    const btnUID = qutor(".profile .detail .user .userid .btn-copy", this.el)
  ...
  writeApps() {
    const eapplist = qutor(".apps .list", this.el)
    eapplist.innerHTML = ""
    this.appGrid = []
    this.appCards = []
    ...
    applist.forEach((k, _index) => {
      ...
      const classList = k.cl ? k.cl[db.onduty] : null
  ...
  navKeyListener() {
    ...
    this.navKeyHandler = (e) => {
      ...
      const currentFocused = eapplist.querySelector(".btn.active")
      if (e.key === "Enter") {
