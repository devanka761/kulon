...
export default function SetNextMap(nextMap, spawnRule = null) {
  const userIds = db.job.players?.length >= 1 ? db.job.players.map((usr) => usr.id) : null
  ...
  Object.keys(nextMap).forEach((key) => {
    ...
    MapList[key].sound = nextMap[key].sound
    MapList[key].configObjects = nextMap[key].configObjects || {}
    MapList[key].walls = nextMap[key].walls || {}
    MapList[key].cutscenes = nextMap[key].cutscenes || {}
    MapList[key].safeZone = nextMap[key].safeZone
    MapList[key].footstep = nextMap[key].footstep
