...
function itemCard(itm_id, s) {
  const tdb = db.trophies.get(itm_id) || {}
  tdb.display = Math.floor(((tdb.taken || 0) / s.taken) * s.display)
  ...
  const ereqs = qutor(".reward .reqs", card)
  const ereward = qutor(".reward", card)
  if (tdb.ts && !tdb.claimed) {
    ...
    ereqs.innerHTML = `<span>${tdb.display || 0}/${s.display}</span>`
  }
...
export default class Trophies {
  ...
  activatedBtn(btn = null) {
    ...
    if (!btn) btn = qutor(`.con-list .card[k-type="${currentPage}"]`, this.el)
  ...
  updateBtn() {
    ...
    const groups = {}
    unreads.forEach((k) => {
      const group = trophy_list[k].group
      groups[group] = true
    })
  ...
  writeItems(ktype = null) {
    ...
    const pageItems = Object.keys(trophy_list).filter((k) => trophy_list[k].group === ktype)
    ...
    const firstCard = elist.querySelector(".card")
    if (firstCard) {
  ...
  navKeyListener() {
    this.navKeyHandler = (e) => {
      ...
      const conList = this.el.querySelector(".con-list")
      const boardList = this.el.querySelector(".board .item-list")

      if (e.key === "ArrowLeft" || e.key === "ArrowRight") {
        const buttons = Array.from(conList.querySelectorAll(".card"))
      ...
      if (!boardList) return
      if (e.key === "Enter") {
        const activeCard = boardList.querySelector(".card.active")
        activeCard?.click()
        return
      }
