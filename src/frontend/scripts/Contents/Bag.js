...
export function transformBagPage(newPage) {
...
export class Bag {
  ...
  createElement() {
    ...
    this.itemContainer = qutor(".item-container", this.el)
    this.itemDetail = qutor(".detail", this.el)
  }
  async btnListener() {
  ...
  navKeyListener() {
    this.navKeyHandler = (e) => {
      ...
      if (e.key === "ArrowUp" || e.key === "ArrowDown") {
        const buttons = Array.from(this.el.querySelectorAll(".con-list .card"))
        ...
        const cards = Array.from(this.el.querySelectorAll(".item-list .card"))
  ...
  updateEconomies() {
    const eperisma = qutor(".economies .eco-perisma i", this.el)
    const etoken = qutor(".economies .eco-token i", this.el)
    this.items = {}
  ...
  writeItems(ktype) {
    ...
    if (elist.firstChild?.classList.contains("card")) {
      elist.firstChild?.click()
    } else {
      this.writeDesc(null)
    }
  }
  writeDesc(s = null) {
    ...
    if (s.expiry) {
      ...
      const itmNote = qutor(".item-note", field)
    ...
    if (item.run) {
      ...
      const itmAct = qutor(".item-actions", field)
