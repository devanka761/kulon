...
export class InputHandler {
  ...
  private blurWalkHandler!: (ev: FocusEvent) => void
  constructor() {}
  ...
  destroy() {
    ...
    window.removeEventListener("blur", this.blurWalkHandler)
  }
  init() {
    ...
    this.blurWalkHandler = () => {
      Object.keys(this.keys).forEach((k) => (this.keys[k as IArrow] = false))
    }
    window.addEventListener("blur", this.blurWalkHandler)
  }
}
