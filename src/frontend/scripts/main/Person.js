...
export class Person {
  constructor(config) {
    ...
    this.animationFrames = {
      "idle-right": { start: 0, end: 5, row: 1 },
      "idle-up": { start: 6, end: 11, row: 1 },
      "idle-left": { start: 12, end: 17, row: 1 },
      "idle-down": { start: 18, end: 23, row: 1 },
  ...
  animate(deltaTime) {
    ...
    const animationName = this.isMoving ? `walk-${this.direction}` : `idle-${this.direction}`
  ...
  facePlayer(playerDirection) {
    ...
    const newDirection = oppositeDirection[playerDirection]
  ...
  isColliding(nextX, nextY, walls, gameObjects) {
    const characterBox = {
    ...
    for (const id in gameObjects) {
      ...
      if (obj === this || !obj.collisionBox || obj.isRemote) {
        continue
      }

      const objBox = {
        x: obj.x + obj.collisionBox.xOffset - 3,
