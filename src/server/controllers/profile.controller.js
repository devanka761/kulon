...
module.exports = {
  find(uid, userid, isMany) {
    ...
    if(isNaN(Number(isMany)) || Number(isMany) < 1) {
      const user = haccount.getUser(uid, userid);
  ...
  addfriend(uid, s) {
    ...
    if(isFriend) return {code:200, data:{user:haccount.getUser(uid,s.id)}};

    if(mdb.req?.includes(s.id)) return this.acceptfriend(uid, s);
    if(udb?.req?.includes(uid)) return {code:200, data: {user:haccount.getUser(uid,s.id)}};
    ...
    db.save('u');
    const dataToSend = {code:200, data:{user:haccount.getUser(uid,s.id)}};
  ...
  unfriend(uid, s) {
    ...
    db.save('u', 'f');
    return {code:200, data:{user:haccount.getUser(uid,s.id)}};
  },
  acceptfriend(uid, s) {
    if(!validate(['id'], s)) return {code:400};
    if(s.id === uid) return {code:400};
    ...
    if(friendkey) return {code:200, data:{user:haccount.getUser(uid,s.id)}};

    if(udb.req?.includes(uid)) db.ref.u[s.id].req = udb.req.filter(key => key !== uid);

    if(!mdb.req || !mdb.req.includes(s.id)) return {code:400, data:{user:haccount.getUser(uid,s.id)}};
    ...
    const dataToSend = {code:200, data:{user:haccount.getUser(uid,s.id), pass: "acceptfriend"}};
  ...
  cancelfriend(uid, s) {
    ...
    const udb = db.ref.u[s.id];
    if(!udb) return {code:400};
    if(!udb.req || !udb.req.includes(uid)) return {code:200, data:{user:haccount.getUser(uid,s.id)}};
    const mdb = db.ref.u[uid];
    if(!mdb.uname) return {code:400, msg:USER_NOT_FOUND};
    if(!mdb.peer) return {code:400, msg:USER_NOT_FOUND};
