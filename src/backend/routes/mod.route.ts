...
import { cdUser, isMod, isUser } from "../main/middlewares"
import { rep } from "../lib/generators"
import { addAssetMap, getEditorMapData, loadProjectMap, remAssetMap, saveProjectMap, sendMail, updateAccess } from "../controller/mod.controller"

const router = express.Router()

router.get("/editor-mapdata", cdUser, isUser, express.json({ limit: "1MB" }), (req: Request, res: Response) => {
  const editor = rep(getEditorMapData())
  return res.status(editor.code).json(editor)
})

router.get("/editor-loadproject/:project_name", cdUser, isUser, express.json({ limit: "1MB" }), (req: Request, res: Response) => {
