...
import ForceClose from "../pages/ForceClose.js";
...
class cloud {
  ...
  async send({id,to,data=null}) {
    ...
    for(const peer of to) {
      ...
      if(this.pair.has(peer)) {
        this.pair.get(peer).send({id, from:db.char.id, data, ts: Date.now()});
        mapPair.d++;
        if(mapPair.d >= mapPair.i) {
          mapPair.i = 0;
          mapPair.d = 0;
  ...
  asend(id, data=null) {
    this.peer.socket._socket.send(JSON.stringify({ku: {id,data}}));
  }
  async reconnect() {
